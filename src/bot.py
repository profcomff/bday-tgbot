...
async def reg_wish(message: types.Message, state: FSMContext):
    ...
    async with db.pool.acquire() as conn:
        await conn.execute(
            ...
            VALUES ($1, $2, $3, $4, NOW())
            """,
            message.from_user.id, data['full_name'], data['birthday'], message.text
        )
    logging.info(f"Регистрация: telegram_id={message.from_user.id}, ФИО={data['full_name']}")
    await state.clear()
...
async def show_ward(message: types.Message):
    ...
    async with db.pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT ward_id FROM users WHERE telegram_id = $1", 
            message.from_user.id
        )
        ...
        ward = await conn.fetchrow(
            "SELECT full_name, birthday, wish FROM users WHERE id = $1", 
            ward_id
        )
...
async def edit_menu_handler(message: types.Message, state: FSMContext):
    ...
    if message.text == 'Изменить ФИО':
        ...
        await message.answer('Введите новую дату рождения (ДД.MM.ГГГГ):', reply_markup=ReplyKeyboardRemove())
        await state.set_state(RegStates.edit_birthday)
    elif message.text == 'Изменить пожелания':
        await message.answer('Введите новые пожелания:', reply_markup=ReplyKeyboardRemove())
    ...
    try:
        async with db.pool.acquire() as conn:
            ...
            for i, pair in enumerate(pairs, 1):
                ...
                bday_formatted = ward_bday.strftime('%d.%m.%Y') if ward_bday else 'не указана'
                
                text += (f"{i}. <b>Даритель:</b> {pair['giver_name']} (ID: {pair['giver_telegram_id']})\n"
...
async def random_distribution(message: types.Message):
    ...
    try:
        async with db.pool.acquire() as conn:
            ...
            await message.answer(f'Успешно распределены {len(pairs)} пар пользователей. Напоминания обновлены.')
...
async def set_pair(message: types.Message):
    ...
    await message.answer(f'Пара назначена: Даритель #{giver_id} → Подопечный #{ward_id}. Напоминания обновлены.')
...
async def set_pair_by_name(message: types.Message):
    ...
    if len(cmd_parts) < 2:
        await message.answer('Использование: /set_name "ФИО дарителя" "ФИО подопечного"')
        return
    ...
    if '"' in full_text:
        ...
        if len(names) < 2:
            await message.answer('Укажите ФИО дарителя и подопечного в кавычках: /set_name "ФИО дарителя" "ФИО подопечного"')
            return
        ...
        if not possible_givers or not possible_wards:
            await message.answer('Не удалось определить дарителя и подопечного. Используйте кавычки: /set_name "ФИО дарителя" "ФИО подопечного"')
            return
    ...
    async with db.pool.acquire() as conn:
        giver = await conn.fetchrow('SELECT id FROM users WHERE full_name = $1', giver_name)
        ward = await conn.fetchrow('SELECT id FROM users WHERE full_name = $1', ward_name)
    ...
    await message.answer(f'Пара назначена: Даритель "{giver_name}" → Подопечный "{ward_name}". Напоминания обновлены.')
...
async def delete_user(message: types.Message):
    ...
    await message.answer(f'Пользователь с Telegram ID {telegram_id} удален. Связи обновлены. Напоминания перепланированы.')
...
async def reset_connections(message: types.Message):
    ...
    try:
        ...
        await message.answer('Ошибка: ID должен быть числом. Используйте `/admin_reset [user_id]` или `/admin_reset all`')
        return
    ...
    await message.answer(f'Связи пользователя #{user_id} и связанных с ним пользователей сброшены. Напоминания обновлены.')
...
async def show_reminders(message: types.Message):
    ...
    for i, job in enumerate(jobs, 1):
        ...
        async with db.pool.acquire() as conn:
            giver_info = await conn.fetchrow('SELECT full_name FROM users WHERE id = $1', giver_id)
            ward_info = await conn.fetchrow('SELECT full_name FROM users WHERE id = $1', ward_id)
        ...
        text += (f"{i}. {run_date.strftime('%d.%m.%Y %H:%M')} (через {days}д {hours}ч)\n"
...
async def menu_command(message: types.Message):
    ...
    try:
        ...
        await message.answer("Произошла ошибка при получении списка команд. Пожалуйста, попробуйте позже.")
